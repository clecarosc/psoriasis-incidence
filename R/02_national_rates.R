...
tasas_nacionales_sexo <- function(ano) {
  psor <- df_casos_py %>% filter(F_ENTRADA == ano)
  regiones_presentes <- unique(psor$servicio)
  
  nacionales_por_sexo <- psor %>% group_by(sexo, edad) %>%  summarise(py = sum(py), cases = sum(cases))
  nacionales_total <- nacionales_por_sexo %>% group_by(edad) %>% summarise(py = sum(py), cases = sum(cases)) %>% add_column(sexo = "T", .before = "edad")
  
  lista_nacional <- list(nacionales_por_sexo, nacionales_total)
  nacional_sexo_total <- bind_rows(lista_nacional)
  
  #parte 2.
  obs <- matrix(nacional_sexo_total$cases, nrow = 3, byrow = TRUE,
  ...
  resultado <- ajuste$adj.strata %>% add_column(year = ano)
  return(resultado) }
